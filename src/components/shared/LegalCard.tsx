    ...
    <Card className={`overflow-hidden hover:shadow-lg transition-shadow cursor-pointer ${
      ...
      <CardContent className={`${viewMode === 'list' ? 'flex-1' : ''} p-4 sm:p-6`}>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2">
            <Badge className={`${getStatusColor(lawyer.status || 'active')} text-xs`}>
              {lawyer.status}
            </Badge>
            ...
            <Badge variant="outline" className="text-xs">
              ...
              {lawyer.barAdmissions || lawyer.location}
            </Badge>
          </div>
          ...
          {(lawyer as any).totalConsultations !== undefined && (
            <div className="grid grid-cols-2 gap-2 text-sm pt-3 border-t">
              <div className="flex items-center">
                <MessageSquare className="h-3 w-3 mr-1 text-gray-500" />
                <span>{(lawyer as any).totalConsultations} consultations</span>
              </div>
              {(lawyer as any).totalEarnings !== undefined && (
                <div className="flex items-center">
                  <DollarSign className="h-3 w-3 mr-1 text-green-600" />
                  <span>${(lawyer as any).totalEarnings}</span>
                </div>
              )}
            </div>
          )}
